  ...
  <section class="array-snapshot-viewer" data-testid="array-snapshot-viewer">
    <header>
      ...
      <div class="snapshot-status">
        <span v-if="snapshot">{{ snapshot.start_index }}..{{ snapshot.start_index + snapshot.count - 1 }} · {{ snapshot.count }} × {{ snapshot.type_name }}</span>
    ...
    <footer v-if="snapshot?.values.length">
      <span>{{ snapshot.start_index }}</span>
      <span>{{ tr('索引', 'Index') }}</span>
      <span>{{ snapshot.start_index + snapshot.values.length - 1 }}</span>
...
function draw(): void {
  ...
  const foreground = chartColor('--fg', '#e6e9ed')
  const border = chartColor('--border', '#343c46')
  const accent = chartColor('--accent', '#58a6d6')
...
<style scoped>
.array-snapshot-viewer { display: grid; grid-template-rows: auto minmax(150px, 1fr) auto; min-height: 190px; overflow: hidden; border-top: 1px solid var(--border); background: var(--surface); }
header, footer { display: flex; align-items: center; justify-content: space-between; gap: 12px; min-height: 34px; padding: 5px 10px; color: var(--muted); font-size: 11px; }
...
.snapshot-status button { min-height: 24px; padding: 2px 8px; border: 1px solid var(--border); background: transparent; color: var(--fg); cursor: pointer; }
.snapshot-chart-wrap { position: relative; min-height: 0; }
canvas { display: block; width: 100%; height: 100%; }
.snapshot-empty { position: absolute; inset: 0; display: grid; place-items: center; color: var(--muted); font-size: 12px; pointer-events: none; }
